...
impl<F: Field> ArithmeticChip<F> {
    ...
    fn eq_constant(
        ...
        cell: AssignedCell<F, F>,
            ...
            |mut region| {
                ...
                // assign the input cell
                let w0 = region.assign_advice(|| "assign w0", self.w0, 0, || cell.value().cloned())?;
                
                // constrain the input
                region.constrain_equal(w0.cell(), cell.cell())?;

                // Set up the gate to check: c0 * w0 + cc = 0
                // Which means: w0 = -cc/c0
                // If we set c0 = 1 and cc = -constant, then w0 = constant
                region.assign_fixed(|| "c0", self.c0, 0, || Value::known(F::ONE))?;
                region.assign_fixed(|| "c1", self.c1, 0, || Value::known(F::ZERO))?;
                ...
                // We don't need to assign w1 and w2 since their coefficients are zero
    ...
    fn eq_cells(
        &self,
        layouter: &mut impl Layouter<F>,
        cell1: AssignedCell<F, F>,
        cell2: AssignedCell<F, F>,
    ) -> Result<(), Error> {
        layouter.assign_region(
            || "eq_cells",
            |mut region| {
                ...
                // assign the input cells
                let w0 = region.assign_advice(|| "assign w0", self.w0, 0, || cell1.value().cloned())?;
                let w1 = region.assign_advice(|| "assign w1", self.w1, 0, || cell2.value().cloned())?;

                // constrain the inputs to be equal
                region.constrain_equal(w0.cell(), cell1.cell())?;
                region.constrain_equal(w1.cell(), cell2.cell())?;
